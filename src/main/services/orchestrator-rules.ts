...
export const GUARDRAIL_PROMPTS: Record<'dev' | 'review' | 'security' | 'simple', string> = {
  dev: [
    ...
    '   Memory system. Use ANAMNESIS_REPO_ID (provided above) as project_id in recall() calls.',
    '   Check for prior decisions, procedures, patterns,',
  ...
  simple: [
    ...
    'If yes to both: print the word DONE on its own line and stop immediately. No further output.',
    'If not: correct it first, then print DONE on its own line and stop.',
    'IMPORTANT: "DONE" must be a plain line of text, not a bullet point. Do not write anything after it.',
    '',
    'COMPLETION SIGNAL (mandatory):',
    'When all work is finished, call the `report_files_changed` MCP tool',
    '(agenthub-kanban server) with:',
    '  - taskId: the task ID provided in your instructions',
    '  - files: array of relative paths of every file you modified',
    '           (use an empty array [] if no files were changed)',
    'ALWAYS call report_files_changed when done — the orchestrator uses this as the',
    'completion signal and will NOT advance to the next task without it.',
    'Call it AFTER your work is complete, not before.',
    'Do NOT print FILES_CHANGED in text — the orchestrator reads from the DB.',
  ].join('\n'),
}
