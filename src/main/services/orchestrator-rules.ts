...
export const GUARDRAIL_PROMPTS: Record<'dev' | 'review' | 'security' | 'simple', string> = {
  ...
  simple: [
    ...
    '',
    'For code tasks (Path B-2): when you have finished modifying files, call the',
    '`report_files_changed` MCP tool (agenthub-kanban server) with:',
    '  - taskId: the task ID provided in your instructions',
    '  - files: array of relative paths of every file you modified',
    'Do NOT print FILES_CHANGED in text — the orchestrator reads from the DB, not your output.',
    'If no files changed, do NOT call the tool — the orchestrator will skip the commit.',
  ].join('\n'),
}
