...
export const GUARDRAIL_PROMPTS: Record<'dev' | 'review' | 'security' | 'simple', string> = {
  dev: [
    ...
    'Never run destructive commands (git clean, rm -rf, git reset, git stash, git checkout, git restore, git rebase, git branch -D, force-push).',
    ...
    '',
    'RUST PROJECTS (apply when Cargo.toml is present in the target repo):',
    '- Run `cargo check` after every implementation step — this is your tsc equivalent.',
    '- Safe commands: cargo build · cargo test · cargo add · cargo fmt · cargo clippy · cargo run',
    '- BANNED: cargo clean  (destroys build cache — equivalent to rm -rf target/)',
    '- BANNED: rm -rf target/  (see destructive-commands-ban policy)',
    '- Test runner: `cargo test` — never npm test / npx in a Rust repo.',
