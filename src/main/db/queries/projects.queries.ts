...
function mapRow(row: Record<string, unknown>): Project {
  return {
    ...
    contextDoc: (row.context_doc as string) ?? null,
    contextDocUpdatedAt: (row.context_doc_updated_at as string) ?? null,
...
export function insertProject(db: Database.Database, input: CreateProjectInput): Project {
  ...
  ).run(id, input.name, input.description ?? null, input.path ?? null, now, now)
  return { id, name: input.name, description: input.description ?? null, path: input.path ?? null, contextDoc: null, contextDocUpdatedAt: null, createdAt: now, updatedAt: now }
}
...
export function updateProject(db: Database.Database, id: string, input: UpdateProjectInput): Project | null {
  ...
  if (input.contextDoc !== undefined) {
    sets.push('context_doc = ?')
    values.push(input.contextDoc)
    sets.push('context_doc_updated_at = ?')
    values.push(now)
  }
